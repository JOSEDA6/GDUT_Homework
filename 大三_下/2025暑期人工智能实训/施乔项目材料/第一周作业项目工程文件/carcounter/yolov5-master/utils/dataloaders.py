...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        """Initializes YOLOv5 loader for image/videos, supporting glob patterns, directories, and lists of paths."""
        ...
        if any(videos):
            ...
            self.cap = None
        assert self.nf > 0, (
            f"No image or videos found in {p}. Supported formats are:\nimage: {IMG_FORMATS}\nvideos: {VID_FORMATS}"
        )
...
class LoadStreams:
    ...
    def __iter__(self):
        """Resets and returns the iterator for iterating over video frames or image in a dataset."""
        self.count = -1
        return self

    def __next__(self):
        """Iterates over video frames or image, halting on thread stop or 'q' key press, raising `StopIteration` when
        done.
        """
        self.count += 1
...
def img2label_paths(img_paths):
    """Generates label file paths from corresponding image file paths by replacing `/image/` with `/labels/` and
    extension with `.txt`.
    """
...
class LoadImagesAndLabels(Dataset):
    """Loads image and their corresponding labels for training and validation in YOLOv5."""
    ...
    def __init__(
        ...
        """Initializes the YOLOv5 dataset loader, handling image and their labels, caching, and preprocessing."""
        self.img_size = img_size
    ...
    def check_cache_ram(self, safety_margin=0.1, prefix=""):
        """Checks if available RAM is sufficient for caching image, adjusting for a safety margin."""
        b, gb = 0, 1 << 30  # bytes of cached image, bytes per gigabytes
    ...
    def cache_labels(self, path=Path("./labels.cache"), prefix=""):
        """Caches dataset labels, verifies image, reads shapes, and tracks dataset integrity."""
        x = {}  # dict
    ...
    def __len__(self):
        """Returns the number of image in the dataset."""
    ...
    def load_mosaic(self, index):
        """Loads a 4-image mosaic for YOLOv5, combining 1 selected and 3 random image, with labels and segments."""
        labels4, segments4 = [], []
        s = self.img_size
    ...
    def load_mosaic9(self, index):
        """Loads 1 image + 8 random image into a 9-image mosaic for augmented YOLOv5 training, returning labels and
        segments.
        """
        labels9, segments9 = [], []
        s = self.img_size
    ...
    def collate_fn(batch):
        """Batches image, labels, paths, and shapes, assigning unique indices to targets in merged label tensor."""
...
class HUBDatasetStats:
    ...
    def process_images(self):
        """Compresses image for Ultralytics HUB across 'train', 'val', 'test' splits and saves to specified
        directory.
        """
        for split in "train", "val", "test":
            if self.data.get(split) is None:
