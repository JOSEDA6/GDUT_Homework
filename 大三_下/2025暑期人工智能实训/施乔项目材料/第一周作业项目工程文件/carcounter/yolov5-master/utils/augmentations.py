...
def normalize(x, mean=IMAGENET_MEAN, std=IMAGENET_STD, inplace=False):
    """
    Applies ImageNet normalization to RGB image in BCHW format, modifying them in-place if specified.
...
def denormalize(x, mean=IMAGENET_MEAN, std=IMAGENET_STD):
    """Reverses ImageNet normalization for BCHW format RGB image by applying `x = x * std + mean`."""
    for i in range(3):
...
def mixup(im, labels, im2, labels2):
    """
    Applies MixUp augmentation by blending image and labels.
...
class LetterBox:
    """Resizes and pads image to specified dimensions while maintaining aspect ratio for YOLOv5 preprocessing."""
