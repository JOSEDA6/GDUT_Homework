...
# Run inference on a directory of image
python detect.py --weights yolov5s.pt --source path/to/image/
...
# Run inference using a glob pattern for image
...
```bash
# Download COCO validation segments split (780MB, 5000 image)
...
```bash
# Run prediction
python segment/predict.py --weights yolov5m-seg.pt --source data/image/bus.jpg
```
...
```bash
# Download ImageNet validation split (6.3GB, 50,000 image)
...
```bash
# Run prediction
python classify/predict.py --weights yolov5s-cls.pt --source data/image/bus.jpg
```
