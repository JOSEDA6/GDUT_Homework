...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        # ​**​*​**START OF YOUR CODE*​**​**
        # 1. 使用仿射变换将图像特征转换为初始隐藏状态 (N, H)
        h0, affine_cache = affine_forward(features, W_proj, b_proj)
        
        # 2. 词嵌入层：将captions_in中的单词索引转换为词向量 (N, T, W)
        ...
        # RNN/LSTM反向传播
        if self.cell_type == "rnn":
            dx_rnn, dh0, dWx_rnn, dWh_rnn, db_rnn = rnn_backward(dh, rnn_cache)
        elif self.cell_type == "lstm":
            dx_rnn, dh0, dWx_rnn, dWh_rnn, db_rnn = lstm_backward(dh, lstm_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(max_length):
            ...
            # 3. RNN/LSTM单步处理
            if self.cell_type == "rnn":
                next_h, _ = rnn_step_forward(x.squeeze(axis=1), prev_h, Wx, Wh, b)
            else:  # LSTM
                next_h, next_c, _ = lstm_step_forward(
                    x.squeeze(axis=1), prev_h, prev_c, Wx, Wh, b
                )
                prev_c = next_c
